...
export const createLiamAsyncStore = (resources) => {
  ...
  resources.forEach(resource => {
    ...
    resource.methods?.forEach(method => {
      apiCalls[method] = (data) => client({ url: resource.url, method: method, data });
    });
    let initialState = {
...
export const useUpdate = (resourceList) => {
  ...
  resourceList.forEach((resource) => {
    output[`update${resource.name.charAt(0).toUpperCase() + resource.name.slice(1)}`] = async (updateFn, body) => {
...
export const useData = (resourcesList) => {
  ...
  const data = useSelector(
    state => {
      const data = {};
      resourcesList.forEach(resource => data[resource.name] = state[resource.name]);
      return data;
    },
    shallowEqual
  );
